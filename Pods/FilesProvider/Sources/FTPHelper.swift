...
public struct FileProviderFTPError: Error {
    ...
    init(message response: String, path: String = "") {
        ...
        self.code = Int(String(message[..<spaceIndex]).trimmingCharacters(in: .whitespacesAndNewlines)) ?? -1
//        self.code = Int(message.substring(to: spaceIndex).trimmingCharacters(in: .whitespacesAndNewlines)) ?? -1
        self.path = path
        if code > 0 {
            self.errorDescription = String(message[..<spaceIndex]).trimmingCharacters(in: .whitespacesAndNewlines)
